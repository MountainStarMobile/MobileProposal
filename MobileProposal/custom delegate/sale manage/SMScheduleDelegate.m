...
@interface SMScheduleDelegate()

@property (nonatomic, retain) NSArray *scheduleArray;

@end

@implementation SMScheduleDelegate

- (id)initWithScheduleArray:(NSArray*)array
{
    self = [super init];
    if (self)
    {
...
- (void)dealloc
{
    [_scheduleArray release], _scheduleArray = nil;
    [super dealloc];
}
...
{
    static NSString *SMScheduleCellIdentifier = @"SMScheduleCellIdentifier";
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:SMScheduleCellIdentifier];
    NSUInteger row = [indexPath row];
    if (cell == nil)
    {
        cell = [[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:SMScheduleCellIdentifier] autorelease];
        [cell.textLabel setFont:[UIFont fontWithName:@"Helvetica" size:14]];
        [cell.detailTextLabel setFont:[UIFont fontWithName:@"Helvetica" size:14]];
    }
    
    scheduleInfo *info = [self.scheduleArray objectAtIndex:row];
...
- (void)createFakeData
{
    ...
    [array addObject:info];
    [info release];
    self.scheduleArray = [NSArray arrayWithArray:array];
}
